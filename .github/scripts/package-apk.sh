...
# Generate OpenWrt APK tracking files (manifest and conffiles)
mkdir -p "${PKG_ROOT}/lib/apk/packages"
(cd "${PKG_ROOT}" && find . -type f -o -type l | sed 's|^\.||' | sort > "lib/apk/packages/${PKG_NAME}.list")
if [ -f "${PKG_ROOT}/etc/config/jodu5164x" ]; then
	echo "/etc/config/jodu5164x" > "${PKG_ROOT}/lib/apk/packages/${PKG_NAME}.conffiles"
	CONF_CSUM=$(sha256sum "${PKG_ROOT}/etc/config/jodu5164x" | awk '{print $1}')
	echo "/etc/config/jodu5164x ${CONF_CSUM}" > "${PKG_ROOT}/lib/apk/packages/${PKG_NAME}.conffiles_static"
fi
...
export pkgname="luci-app-jodu5164x-status"
[ -n "${IPKG_INSTROOT}" ] || {
...
docker run --rm \
	...
	alpine:edge sh -c "
		...
		SOURCE_DATE_EPOCH=0 apk mkpkg \
			...
			--info \"url:https://github.com/MrManiesh/luci-app-jodu5164x-onyx\" \
